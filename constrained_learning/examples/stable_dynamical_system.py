...
from constrained_learning.constraints import DEQC, CIEQC, ObjFct
from constrained_learning.learner import CELM
import constrained_learning.utils as utils

# build dataset
demo_0 = lasa.DataSet.BendedLine.demos[2]
x = demo_0.pos.T
y = demo_0.vel.T
...
x -= x[-1, :]
y *= scaler.scale_  # to preserve the relationship between pos and vel, correctly scale vel
...
num_points = 10000
xx, yy = np.meshgrid(np.linspace(region.lower_bounds[0], region.upper_bounds[0], int(np.sqrt(num_points))),
                     np.linspace(region.lower_bounds[1], region.upper_bounds[1], int(np.sqrt(num_points))))
...
plt.plot(x[:, 0], x[:, 1], color='black', label='Dataset', linewidth=3)
plt.plot(p[:, 0], p[:, 1], '-', color='green', label='Reproduction', linewidth=3)
...
plt.streamplot(xx, yy, uu, vv, color=norm, linewidth=2, density=1.3, cmap='viridis')
